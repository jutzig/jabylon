...
public class PropertyEditorSinglePanel extends
        BasicResolvablePanel<PropertyFileDescriptor> {

    private static final long serialVersionUID = 1L;
    private static final Logger logger = LoggerFactory
            .getLogger(PropertyEditorSinglePanel.class);
    ...
    public PropertyEditorSinglePanel(PropertyFileDescriptor object,
            PageParameters parameters) {
        super("content", object, parameters);

        PropertyListMode mode = PropertyListMode.getByName(parameters.get(
                "mode").toString("ALL"));
    ...
    private void createModels(IModel<PropertyFileDescriptor> model,
            String targetKey, PropertyListMode mode) {
        ...
        for (Property property : templateFile.getProperties()) {
            ...
            PropertyPair pair = new PropertyPair(property, translation,
                    descriptor.getVariant(), descriptor.cdoID());
                ...
                if (mode.apply(pair, reviews.get(pair.getKey()))) {
                    if (main != null) {
                        ...
                        break;
                            ...
                            || (pair.getKey() != null && pair.getKey().equals(
                                    targetKey))) {
                        // this is the one we need to edit
                        main = pair;
                    } else
    ...
    private void buildComponentTree(PropertyPair previous, PropertyPair main,
            PropertyPair next) {
        Form<Property> pairForm = new StatelessForm<Property>(
                "properties-form", Model.of(main.getTranslation())) {
        ...
        final WebMarkupContainer templatePanel = new WebMarkupContainer(
                "template-area");
        templatePanel.setOutputMarkupId(true);
        pairForm.add(templatePanel);
        final WebMarkupContainer translationPanel = new WebMarkupContainer(
                "translation-area");
        translationPanel.setOutputMarkupId(true);
        pairForm.add(translationPanel);

        final Label translationLabel = new Label("translation-label",
                new PropertyModel<PropertyPair>(main, "translated"));

        pairForm.add(new Label("key-label", key));
        pairForm.add(translationLabel);

        TextArea<PropertyPair> textArea = new TextArea<PropertyPair>(
                "template", new PropertyModel<PropertyPair>(main, "original"));
        templatePanel.add(textArea);

        textArea = new TextArea<PropertyPair>("template-comment",
                new PropertyModel<PropertyPair>(main, "originalComment"));
        templatePanel.add(textArea);

        textArea = new TextArea<PropertyPair>("translation-comment",
                new PropertyModel<PropertyPair>(main, "translatedComment"));
        translationPanel.add(textArea);

        textArea = new TextArea<PropertyPair>("translation",
                new PropertyModel<PropertyPair>(main, "translated"));
        translationPanel.add(textArea);
    ...
    private Multimap<String, Review> buildReviewMap(
            PropertyFileDescriptor object) {
        EList<Review> reviews = object.getReviews();
        Multimap<String, Review> reviewMap = ArrayListMultimap.create(
                reviews.size(), 2);
    ...
    private void addLinkList(final PropertyListMode currentMode) {
        List<PropertyListMode> values = Arrays
                .asList(PropertyListMode.values());
        ListView<PropertyListMode> mode = new ListView<PropertyListMode>(
                "view-mode", values) {
